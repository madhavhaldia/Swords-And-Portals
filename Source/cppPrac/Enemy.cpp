...
void AEnemy::CombatSphereOnOverlapBegin(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if (OtherActor && Alive())
	{
		...
		if (Main)
		{
			...
			float AtackTime = FMath::FRandRange(AttackMinTime, AttackMaxTime);
			GetWorldTimerManager().SetTimer(AttackTimer, this, &AEnemy::Attack, AtackTime);
			Attack();
		}
...
void AEnemy::MoveToTarget(class AMainCharacter* Target)
{
	...
	if (AIController)
	{
		FAIMoveRequest MoveRequet;
		MoveRequet.SetGoalActor(Target);
		MoveRequet.SetAcceptanceRadius(5.0f);
		FNavPathSharedPtr NavPath;  
		AIController->MoveTo(MoveRequet, &NavPath);
		/**
		...
		for (auto Point : PathPoints)
		{
			...
			UKismetSystemLibrary::DrawDebugSphere(this, Location, 25.f, 8, FLinearColor::Yellow, 10.f, 0.5f);
		}
		*/
	}
}
